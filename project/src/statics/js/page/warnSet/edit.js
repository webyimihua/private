...
layui.use(['form','layer','tools'],function(){
    ...
    form.on("submit(editAlarm)",function(data){
        var data = data.field;
        editAlarmData(data)
        return false;
    })
    ...
    function findMonitorDimension(){
     ...
     function editMonitorPoints(MonitorPoint){
    ...
    form.on('select(editMonitorBody)',function(data){
    	MonitorPoint['objectId'] = data.value;
        editMonitorPoints(MonitorPoint)
    });
    form.on('select(editDimension)',function(data){
